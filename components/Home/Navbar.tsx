...
interface NavbarProps {
  onNavigate: (page: 'home' | 'tourism' | 'departments') => void;
  currentPage: 'home' | 'tourism' | 'departments';
}

const Navbar: React.FC<NavbarProps> = ({ onNavigate, currentPage }) => {
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);

  const handleNavClick = (page: 'home' | 'tourism' | 'departments', e: React.MouseEvent) => {
    e.preventDefault();
    onNavigate(page);
  ...
  const NavItem = ({ label, page, isExternal = false }: { label: string, page?: 'home' | 'tourism' | 'departments', isExternal?: boolean }) => {
        ...
        <div className="container mx-auto px-4 md:px-6">
            <div className="flex justify-between md:justify-center items-center h-14 md:h-12">
                ...
                <div className="hidden md:flex items-center justify-center w-full flex-wrap">
                    ...
                    <Separator />
                    <NavItem label="GAD DATABASE" isExternal />
                    <Separator />
                    <NavItem label="LGU EO" isExternal />
                    <Separator />
                    <NavItem label="PROCUREMENT" isExternal />
                    <Separator />
